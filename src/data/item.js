...
class DalgonaJam extends IProducible {
    constructor(count) {
        super("DalgonaJam", "달고나 잼", count, 20 * 60, 1, [new JellyBean(6), new JellyBerry(1)]);
    }
}
...
class SwanFeatherDreamcatcher extends IProducible {
    constructor(count) {
        super("SwanFeatherDreamcatcher", "백조깃털 드림캐처", count, 3.5*60*60, 1, [new CottonCandyWool(6), new SoftOmurice(1)]);
    }
}
...
class ReliableRyeBread extends IProducible {
    constructor(count) {
        super("ReliableRyeBread", "든든한 호밀빵", count, 12*60, 1, [new JellyBeanJam(1), new BiscuitPowder(2)]);
    }
}
class SweetAndChewyJamPie extends IProducible {
    constructor(count) {
        super("SweetAndChewyJamPie", "달콤쫀득 잼파이", count, 20*60, 1, [new JellyBean(6), new BiscuitPowder(3)]);
    }
}
class GinkgoPocachia extends IProducible {
    constructor(count) {
        super("GinkgoPocachia", "은행 포카치아", count, 30*60, 1, [new BiscuitPowder(6), new AcornLamp(1)]);
    }
}
class SugarCoatedDonut extends IProducible {
    constructor(count) {
        super("SugarCoatedDonut", "슈가코팅 도넛", count, 60*60, 1, [new BiscuitPowder(5), new TwinklingGlassPlate(1)]);
    }
}
class SoftCastella extends IProducible {
    constructor(count) {
        super("SoftCastella", "폭신 카스테라", count, 3*60*60, 1, [new SugarCubePiece(20), new MilkyMilk(8)]);
    }
}
class GoldrichCroissant extends IProducible {
    constructor(count) {
        super("GoldrichCroissant", "골드리치 크로와상", count, 6*60*60, 1, [new MilkyMilk(15), new Butter(1)]);
    }
}
...
class HotJellyStew extends IProducible {
    constructor(count) {
        super("HotJellyStew", "따끈따끈 젤리스튜", count, 18*60, 1, [new JellyBean(4), new JellyBerry(1)]);
    }
}
class GummyBearBurger extends IProducible {
    constructor(count) {
        super("GummyBearBurger", "곰젤리 버거", count, 22*60, 1, [new JellyBean(10), new SweetAndChewyJamPie(1)]);
    }
}
class CandyCreamPasta extends IProducible {
    constructor(count) {
        super("CandyCreamPasta", "캔디크림 파스타", count, 50*60, 1, [new BiscuitPowder(7), new Cream(1)]);
    }
}
class SoftOmurice extends IProducible {
    constructor(count) {
        super("SoftOmurice", "폭신폭신 오므라이스", count, 1.5*60*60, 1, [new JellyBerry(6), new GinkgoPocachia(1)]);
    }
}
class CombinationPizzaJelly extends IProducible {
    constructor(count) {
        super("CombinationPizzaJelly", "콤비네이션 피자젤리", count, 3.5*60*60, 1, [new JellyBerry(8), new CandyFlower(4)]);
    }
}
class LuxuriousJellyBeanSetMenu extends IProducible {
    constructor(count) {
        super("LuxuriousJellyBeanSetMenu", "고급스러운 젤리빈 정식", count, 7*60*60, 1, [new JellyBean(25), new LollipopFlowerBasket(3)]);
    }
}
...
class BiscuitPlantPot extends IProducible {
    constructor(count) {
        super("BiscuitPlantPot", "비스킷 화분", count, 15*60, 1, [new PineConeBirdDoll(2), new BiscuitPowder(4)]);
    }
}
class TwinklingGlassPlate extends IProducible {
    constructor(count) {
        super("TwinklingGlassPlate", "반짝반짝 유리판", count, 27*60, 1, [new SugarCubePiece(12), new HotJellyStew(1)]);
    }
}
class GlitteringSaekdongBeads extends IProducible {
    constructor(count) {
        super("GlitteringSaekdongBeads", "반짝이는 색동구슬", count, 2*60*60, 1, [new GummyBearBurger(1), new CottonCandyWool(1)]);
    }
}
class RainbowDessertBowl extends IProducible {
    constructor(count) {
        super("RainbowDessertBowl", "무지갯빛 디저트 보울", count, 5*60*60, 1, [new SugarCubePiece(24), new PomegranateJam(2)]);
    }
}
...
class CandyFlower extends IProducible {
    constructor(count) {
        super("CandyFlower", "캔디꽃", count, 20*60, 1, [new JellyBerry(2), new BiscuitPlantPot(1)]);
    }
}
class HappyFlowerPot extends IProducible {
    constructor(count) {
        super("HappyFlowerPot", "행복한 꽃화분", count, 20*60, 1, [new JellyBerry(4), new SugarCubePiece(10)]);
    }
}
class CandyFlowerBouquet extends IProducible {
    constructor(count) {
        super("CandyFlowerBouquet", "캔디꽃다발", count, 60*60, 1, [new JellyBerry(5), new CandyCreamPasta(1)]);
    }
}
class LollipopFlowerBasket extends IProducible {
    constructor(count) {
        super("LollipopFlowerBasket", "롤리팝 꽃바구니", count, 2.5*60*60, 1, [new JellyBean(18), new ReliableRyeBread(3)]);
    }
}
class GlassFlowerBouquet extends IProducible {
    constructor(count) {
        super("GlassFlowerBouquet", "유리꽃 부케", count, 4.5*60*60, 1, [new JellyBean(20), new CloudCandyCushion(2)]);
    }
}
class BrilliantYogurtWreath extends IProducible {
    constructor(count) {
        super("BrilliantYogurtWreath", "찬란한 요거트 화환", count, 7.5*60*60, 1, [new JellyBean(30), new CombinationPizzaJelly(2)]);
    }
}
...
class Cream extends IProducible {
    constructor(count) {
        super("Cream", "크림", count, 29*60, 1, [new SweetJellyJam(1), new MilkyMilk(2)]);
    }
}
class Butter extends IProducible {
    constructor(count) {
        super("Butter", "버터", count, 2.5*60*60, 1, [new SugarCubePiece(15), new MilkyMilk(5)]);
    }
}
class HandmadeCheese extends IProducible {
    constructor(count) {
        super("HandmadeCheese", "수제 치즈", count, 4.5*60*60, 1, [new MilkyMilk(10), new CuckooCuckooClock(1)]);
    }
}
...
class SoftAndFuzzyBubbleTea extends IProducible {
    constructor(count) {
        super("SoftAndFuzzyBubbleTea", "몽글몽글 버블티", count, 3 * 60 * 60, 1, [new CottonCandyWool(1), new SugarCoatedDonut(1)]);
    }
}
class SweetBerryAde extends IProducible {
    constructor(count) {
        super("SweetBerryAde", "스윗베리 에이드", count, 6.5 * 60 * 60, 1, [new JellyBerry(10), new GlitteringSaekdongBeads(2)]);
    }
}
...
class CloudCandyCushion extends IProducible {
    constructor(count) {
        super("CloudCandyCushion", "구름사탕 쿠션", count, 1.5 * 60 * 60, 1, [new BiscuitPowder(8), new CottonCandyWool(1)]);
    }
}
class GummyBearCottonDoll extends IProducible {
    constructor(count) {
        super("GummyBearCottonDoll", "곰젤리 솜인형", count, 4 * 60 * 60, 1, [new CandyFlowerBouquet(2), new CottonCandyWool(1)]);
    }
}
class DragonFruitCottonDoll extends IProducible {
    constructor(count) {
        super("DragonFruitCottonDoll", "용과 드래곤 솜인형", count, 7 * 60 * 60, 1, [new CottonCandyWool(2), new CreamRootBeer(2)]);
    }
}
...
class CreamRootBeer extends IProducible {
    constructor(count) {
        super("CreamRootBeer", "크림 루트비어", count, 2.5 * 60 * 60, 1, [new BiscuitPowder(10), new HappyFlowerPot(1)]);
    }
}
class RedBerryJuice extends IProducible {
    constructor(count) {
        super("RedBerryJuice", "레드베리 주스", count, 6.5 * 60 * 60, 1, [new JellyBerry(12), new GummyBearCottonDoll(1)]);
    }
}
class VintageWildBottle extends IProducible {
    constructor(count) {
        super("VintageWildBottle", "빈티지 와일드 보틀", count, 8 * 60 * 60, 1, [new RollCakeWood(10), new HappyFlowerPot(1)]);
    }
}
...
class SpookyMuffin extends IProducible {
    constructor(count) {
        super("SpookyMuffin", "으스스 머핀", count, 3.5 * 60 * 60, 1, [new BiscuitPowder(12), new MilkyMilk(7)]);
    }
}
class FreshStrawberryCake extends IProducible {
    constructor(count) {
        super("FreshStrawberryCake", "생딸기 케이크", count, 6 * 60 * 60, 1, [new BiscuitPowder(14), new SoftAndFuzzyBubbleTea(2)]);
    }
}
class PartyPartyChiffonCake extends IProducible {
    constructor(count) {
        super("PartyPartyChiffonCake", "파티파티 쉬폰케이크", count, 8 * 60 * 60, 1, [new BiscuitPowder(18), new HandmadeCheese(2)]);
    }
}
...
class GlazedRing extends IProducible {
    constructor(count) {
        super("GlazedRing", "글레이즈드 링", count, 5 * 60 * 60, 1, [new BiscuitPowder(12), new SoftCastella(1)]);
    }
}
class RubyBerryBrooch extends IProducible {
    constructor(count) {
        super("RubyBerryBrooch", "루비베리 브로치", count, 7.5 * 60 * 60, 1, [new JellyBerry(16), new SwanFeatherDreamcatcher(1)]);
    }
}
class RoyalGummyBearCrown extends IProducible {
    constructor(count) {
        super("RoyalGummyBearCrown", "로얄 곰젤리 크라운", count, 8 * 60 * 60, 1, [new CottonCandyWool(3), new GlassFlowerBouquet(1)]);
    }
}
