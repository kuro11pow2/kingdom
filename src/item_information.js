...
window.addEventListener("load", () => {
    ...
    tableHead.appendChild(CreateRow("th", ["이름", "생산 단계", "소요 시간 총합 (초/개당)", "소요 코인 총합 (코인/개당)", "납품 점수 (점/개당)", "직접 요구 재료 (개당)"]));
    for (let item of Object.values(ItemFactory)) {
        ...
        let str = new Array(inst.krName, inst.order, Math.round(inst.totalTimePerRequired*100)/100, Math.round(inst.totalCoinPerRequired*100)/100, inst.deliveryScore, mateiralsStr);
